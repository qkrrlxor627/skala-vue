...
const riskLevels = ref([
  {
    ...
    badge: '작업 중단 권고',
    tone: 'danger',
    condition: '강수확률 70% 이상 또는 최고기온 40°C 이상',
    action: '야외 작업 중단, 자재 결박 및 배수로 점검, 그늘·휴게 공간으로 대피',
  },
  {
    id: 'risk_warn',
    badge: '주의 권고',
    tone: 'warn',
    condition: '강수확률 40% 이상 또는 최고기온 33°C 이상',
    action: '고소, 크레인 작업 재검토, 철수 동선 사전 확인, 시간당 10~15분 휴식과 음용수 비치',
  },
  {
    id: 'risk_safe',
    badge: '작업 가능',
    tone: 'safe',
    condition: '강수확률 40% 미만이고 최고기온 33°C 미만',
    action: '정상 작업 진행',
  },
])
...
const techStack = ref([
  ...
  },
  {
    ...
    desc: '5일/3시간 예보를 기반으로 현재 기상과 24시간 강수확률, 최고기온을 제공합니다.',
  },
])
</script>
...
<template>
  <div class="about">
    ...
    <p class="about__lead">
      ...
      받아 강수확률과 기온을 기준으로 야외 작업 가능 여부를 3단계로 판정합니다. 사업장명과
      지역명으로 검색하고, 섭씨와 화씨를 전환하고, 자주 보는 사업장을 즐겨찾기에 담을 수 있습니다.
    </p>
    ...
    <p class="about__note-sm">
      24시간(3시간 간격 8건) 예보에서 <strong>최고 강수확률과 최고기온</strong>을 각각 판정한 뒤
      <strong>더 위험한 쪽 등급</strong>을 최종 등급으로 채택합니다. 야외 작업은 강수 시작 전 철수
      준비 시간이 필요하고, 폭염은 비가 오지 않아도 단독으로 온열질환 위험을 만들기 때문입니다.
      배지에는 등급을 결정한 사유(폭염 / 강수)를 함께 표시합니다.
    </p>
